...
def _sync_reasona_config(workdir: Path, path: Path) -> None:
    ...
    # Copying the TEMPLATE alone is not enough -- `.bernstein/bernstein.yaml`
    # itself (the file `bernstein run` actually reads) is normally
    # regenerated by `ensure_bernstein_yaml()`, but that call only happens
    # from `plan_compile.write_plan_yaml()`, at cycle-0 dispatch. A unit
    # resuming past cycle-0 (`dev_already_dispatched()` already True, the
    # common case for review/scan/final-audit on every call after the
    # first) never reaches that call again -- so if `.bernstein/
    # bernstein.yaml` is ever missing or stale in an EXISTING worktree
    # (deleted by hand, a cleanup sweep, anything), nothing else in this
    # path would put it back. Regenerating it here too closes that gap
    # directly, rather than relying on cycle-0 having done it once.
    from reasona_dev.bernstein_config import ensure_bernstein_yaml

    ensure_bernstein_yaml(path)
