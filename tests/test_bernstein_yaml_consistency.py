...
Reads `.bernstein/bernstein.yaml` -- this repo commits it there directly
(not under `.reasona/`), because Bernstein's own config loader checks that
location FIRST for bare `bernstein` / `bernstein doctor` (no `--seed`
override exists for those; only `bernstein run` has one). `resolve_all(
workdir=REPO_ROOT)` resolves against this repo's OWN committed
`.reasona/reasona.yaml` (project-local beats global), so this test's
outcome doesn't depend on what happens to be in the machine's
`~/.reasona/reasona.yaml`.
"""
...
def test_role_model_policy_providers_match_resolved_adapters():
    bernstein_yaml = yaml.safe_load((REPO_ROOT / ".bernstein" / "bernstein.yaml").read_text())
    policy = bernstein_yaml["role_model_policy"]

    resolved = resolve_all(workdir=REPO_ROOT)  # this repo's own committed .reasona/reasona.yaml
