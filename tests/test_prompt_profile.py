...
def test_default_profile_when_nothing_set():
    assert resolve_profile_name(env={}) == "rust-dev"
...
def test_this_repos_own_committed_profile_is_readable(tmp_path, rust_dev_prompts):
    """`.reasona/prompts/rust-dev/` is a real, committed profile now -- not a
    copy inside the installed package."""
    text = resolve_prompt("review", profile="rust-dev", workdir=tmp_path)
...
def test_project_local_beats_global(tmp_path, monkeypatch):
    ...
    (fake_home / ".reasona" / "prompts" / "rust-dev").mkdir(parents=True)
    (fake_home / ".reasona" / "prompts" / "rust-dev" / "review.md").write_text("global\n")
    monkeypatch.setattr("pathlib.Path.home", lambda: fake_home)

    workdir = tmp_path / "target-repo"
    prompt_dir = workdir / ".reasona" / "prompts" / "rust-dev"
    prompt_dir.mkdir(parents=True)
    (prompt_dir / "review.md").write_text("project-local override\n")

    assert resolve_prompt("review", profile="rust-dev", workdir=workdir) == "project-local override\n"
...
def test_per_role_shadowing_not_whole_profile_shadowing(tmp_path, monkeypatch):
    ...
    fake_home = tmp_path / "fake-home"
    g = fake_home / ".reasona" / "prompts" / "rust-dev"
    ...
    workdir = tmp_path / "target-repo"
    local = workdir / ".reasona" / "prompts" / "rust-dev"
    local.mkdir(parents=True)
    (local / "review.md").write_text("local review\n")

    assert resolve_prompt("review", profile="rust-dev", workdir=workdir) == "local review\n"
    assert resolve_prompt("bugbot", profile="rust-dev", workdir=workdir) == "global bugbot\n"
...
def test_no_layer_present_returns_none_rather_than_a_packaged_default(tmp_path, monkeypatch):
    ...
    for role in ("review", "recheck", "bugbot", "compliance", "final_audit"):
        assert resolve_prompt(role, profile="rust-dev", workdir=workdir) is None
...
def test_available_profiles_merges_both_layers(tmp_path, monkeypatch):
    ...
    assert found == {"rust-dev": ["bugbot", "review"], "rust": ["review"]}
...
def test_resolve_prompt_never_falls_back_to_a_different_profile(tmp_path):
    ...
    # slide back to "rust-dev" -- that would be a silent substitution.
    workdir = tmp_path / "target-repo"
    workdir.mkdir()
